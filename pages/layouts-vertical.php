...

{
    ...
    $sqlList = "SELECT count(*)as tc FROM " . DB_PREFIX . "patient_master WHERE pm_del = 0";
    ...
    $result = $qryList->fetch(PDO::FETCH_ASSOC);
...
{
    $from_new_query = "FROM tbl_user_master WHERE ut_id IN ('1','2') AND um_del = '0' ";
...
// Function to populate EECP count
function get_all_eecp_count(PDO $DB)

{

    $sqlList = "SELECT count(*) as tc FROM tbl_patient_package_master tppm LEFT JOIN `tbl_patient_master` tpm ON tppm.patient_unique_id = tpm.patient_unique_id LEFT JOIN `tbl_hospital_master` thm ON thm.hm_id = tpm.hm_id WHERE tppm.package_type = '2' AND tppm.tppm_del = '0' AND (tppm.tppm_status = '1' OR tppm.tppm_status = '0')";
...
// Function to populate SDT count
function get_all_sdt_count(PDO $DB)

{

    $sqlList = "SELECT count(*) as tc FROM tbl_patient_package_master tppm LEFT JOIN `tbl_patient_master` tpm ON tppm.patient_unique_id = tpm.patient_unique_id LEFT JOIN `tbl_hospital_master` thm ON thm.hm_id = tpm.hm_id WHERE tppm.package_type = '1' AND tppm.tppm_del = '0' AND (tppm.tppm_status = '1' OR tppm.tppm_status = '0')";
